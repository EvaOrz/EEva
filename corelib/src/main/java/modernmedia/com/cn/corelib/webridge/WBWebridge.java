...
public class WBWebridge {
	...
	public WBWebridge(WBWebView webView, WBWebridgeListener listener) {
		mWebView = webView;
	...
	public void postMessage(final String json) {
		...
		// {"eval":{"command":command, "params":params} };
		JSONObject obj = null;
		try {
	 ...
	 */
	@JavascriptInterface
	private void jsToNative(JSONObject evalObj) {
		// 需要执行的native方法名
		final String command = evalObj.optString("command");
		// 需要执行的native方法参数
		Object params = evalObj.opt("params");
		if (JSONObject.NULL.equals(evalObj)) {
			evalObj = null;
		}
		final Object parmaObj = evalObj;
	...
	private void onFetchResult(Object result, String command, int sequence) {
		...
		if (!(result instanceof String)) {
			...
			try {
				...
				System.out.println("测试异步" + result.toString());
