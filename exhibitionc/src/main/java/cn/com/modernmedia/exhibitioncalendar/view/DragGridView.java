...
public class DragGridView extends GridView {
	...
	private OnItemLongClickListener onLongClickListener = new OnItemLongClickListener() {
		...
		public boolean onItemLongClick(AdapterView<?> parent, View view,
			...
			view.getParent().requestDisallowInterceptTouchEvent(true);// 通知父控件勿拦截本控件touch事件

			final TagInfo t = (TagInfo) view.getTag();
			if (t != null && t.isCheck()) {// 排序状态
				// 记录长按item位置
				...
				// 设置被长按item不显示
				((DragCityAdapter) getAdapter()).hideView(position);
				return true;
			} else if (!((CityPickActivity) context).ischeck) {// 编辑状态待选区长按先判断登陆
				((CityPickActivity) context).isLogined();
			}
			return false;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		// dragImageView处于被拖动时，更新dragImageView位置
			...
			if ((currDraggedPosition != AdapterView.INVALID_POSITION)
					&& (currDraggedPosition != preDraggedOverPositon)) {
				if (((DragCityAdapter) getAdapter()).swapView(
		...
		// 释放dragImageView
				...
				&& (isViewOnDrag == true)) {
			((DragCityAdapter) getAdapter()).showHideView();
			if ((Integer) dragImageView.getTag() == DRAG_IMG_SHOW) {
				windowManager.removeView(dragImageView);
				dragImageView.setTag(DRAG_IMG_NOT_SHOW);
			}
